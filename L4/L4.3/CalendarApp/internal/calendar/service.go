...
type service struct {
	...
	archive      map[int]Event
	nextID       int
	reminderChan chan<- Event
}

func NewService(reminderChan chan<- Event) Service {

	return &service{
		events: make(map[int]Event),

		archive: make(map[int]Event),
...
func (s *service) CreateEvent(req CreateEventRequest) (*Event, error) {
	...
	if s.reminderChan != nil {
		s.reminderChan <- event
	}
...
func (s *service) ArchiveOldEvents() {
